...
namespace rat {
	namespace {
		const Reg kIntArgRegs[6] = {RDI, RSI, RDX, RCX, R8, R9};
		const U32 kMaxIntArgs = 6;
		const U32 kMaxXmmArgs = 8;

		const U32 kGpSaveBytes = kMaxIntArgs * 8;
		...
		struct X86Emitter {
			...
			B32 variadic = false;
			I32 saveArea = 0;
			U32 namedGp = 0, namedFp = 0;
			I32 overflowOff = 16;

			List<U32> blockOffset;
			...
			void fstpX87Slot(Node* n) { a.fstpT(RBP, slotOf(n)); }
			...
			void maskToBits(U32 bits, Reg r0, Reg r1 = RAX) {
				if (bits == 0 || bits >= 64)
					return;
			...
			void vaRegOrStack(I32 offDisp, U32 limit, I32 regStep) {
	...
	static void emitModule(const Module& mod, std::ostream& os) {
		ElfObject elf;
	...
	void emitX86(const Module& mod, std::ostream& os) { emitModule(mod, os); }
