...
namespace rat::cc {
	...
	Emitter::Value Emitter::emitUnary(Function& fn, const Expr* e) {
		...
		if (isComplexType(v.type) &&
				(e->unary.op == ExprOp::Pos || e->unary.op == ExprOp::Neg))
		...
		case ExprOp::Not:
			...
			if (isComplexType(v.type))
				return {fromBool(fn, fn.eq(toBool(fn, v), fn.constInt(i32, 1))),
								ctInt()};
			if (isFloating(v.type))
				return {fromBool(fn, fn.compare(Opcode::FEq, v.node,
																				fn.constFloat(irType(v.type), 0.0))),
								ctInt()};
			return {fromBool(fn, fn.eq(v.node, fn.constInt(irType(v.type), 0))),
							ctInt()};
		default:
	...
	Emitter::Value Emitter::emitStmtExpr(Function& fn, const Expr* e) {
		...
		for (U32 i = 0; i < stmts.size(); ++i) {
			...
			if (fn.blockFinished() && child->kind != StmtKind::Label &&
					child->kind != StmtKind::Case && child->kind != StmtKind::Default &&
					!containsLabel(child)) {
	...
	Emitter::Value Emitter::emitExpr(Function& fn, const Expr* e) {
		...
		case ExprKind::Cast: {
			...
			if (isPointer(v.type) && !isPointer(castTy) && !isFloating(castTy) &&
					!castTy.isVoid) {
				if (castTy.bits == 1)
		...
		case ExprKind::VaArg: {
			...
			if (!ap.node)
				return {};
			Type* fetched =
					isStruct(e->vaArg.type) ? mod.getPtr() : irType(e->vaArg.type);
	...
	Emitter::Value Emitter::emitComparison(Function& fn, ExprOp op, Value lhs,
																				 Value rhs) {
	...
	Emitter::Value Emitter::emitBinary(Function& fn, const Expr* e) {
		...
		if (isComplexType(lhs.type) || isComplexType(rhs.type)) {
			...
			if (op == ExprOp::Eq || op == ExprOp::Ne) {
				...
				Node* re =
						fn.compare(Opcode::FEq, complexReal(fn, lc), complexReal(fn, rc));
				Node* im =
						fn.compare(Opcode::FEq, complexImag(fn, lc), complexImag(fn, rc));
	...
	Emitter::Value Emitter::emitCompoundLit(Function& fn, const Expr* e) {
		{
			...
			if (e->compound.isArray) {
				...
				Node* slot = isAggregate(ty)
												 ? allocBytes(fn, total)
												 : fn.alloc(mod.getArray(elemTy, (U32)count));
	...
	B32 Emitter::exprRefersTo(const Expr* e, const String& name) const {
		...
		case ExprKind::Binary:
			return exprRefersTo(e->binary.lhs, name) ||
						 exprRefersTo(e->binary.rhs, name);
		case ExprKind::Ternary:
			return exprRefersTo(e->ternary.cond, name) ||
						 exprRefersTo(e->ternary.whenTrue, name) ||
						 exprRefersTo(e->ternary.whenFalse, name);
		case ExprKind::Comma:
			return exprRefersTo(e->comma.lhs, name) ||
						 exprRefersTo(e->comma.rhs, name);
		case ExprKind::Cast:
	...
	void Emitter::collectSwitchCases(const Stmt* s, List<const Stmt*>& cases,
																	 const Stmt*& def) {
		if (!s)
			return;
		switch (s->kind) {
	...
	B32 Emitter::declareStatic(Function& fn, const Declarator& d) {
		String sym =
				"__ratcc_static" + std::to_string(staticCounter++) + "_" + *d.name;
		...
		if (d.init) {
			if (d.init->kind == ExprKind::InitList) {
				...
				if (els.size() > 1) {
					...
					if (!iv.node)
						return false;
					fn.store(offsetPtr(fn, slot, byteSize(re)),
									 convert(fn, iv.node, iv.type, re));
				}
			} else {
	...
	B32 Emitter::emitArrayDecl(Function& fn, const Declarator& d) {
		...
		if (d.init && d.init->kind == ExprKind::StrLit) {
			...
			for (I64 i = 0; i < count; ++i) {
				...
				fn.store(offsetPtr(fn, slot, (U64)i * elemSize),
								 fn.constInt(elemTy, val));
			}
			declare(*d.name, Local{0, slot, d.type, true, true, (U32)count});
			return true;
		}
	...
	B32 Emitter::emitOneDecl(Function& fn, const Declarator& d) {
		...
		if (memVars.count(*d.name)) {
			...
			declare(*d.name, Local{fn.declareLocal(*d.name, init), nullptr, d.type,
														 false, false});
		}
		return true;
	}
	...
	void Emitter::collectAddrTakenExpr(const Expr* e) {
		...
		case ExprKind::Unary:
			if (e->unary.op == ExprOp::Addr &&
					e->unary.operand->kind == ExprKind::Ident)
	...
	B32 Emitter::containsLabelInExpr(const Expr* e) {
		...
		case ExprKind::Unary:
			...
			break;
		case ExprKind::Binary:
			if (containsLabelInExpr(e->binary.lhs) ||
					containsLabelInExpr(e->binary.rhs))
				return true;
			break;
		case ExprKind::Ternary:
			if (containsLabelInExpr(e->ternary.cond) ||
					containsLabelInExpr(e->ternary.whenTrue) ||
					containsLabelInExpr(e->ternary.whenFalse))
				return true;
			break;
		case ExprKind::Comma:
			if (containsLabelInExpr(e->comma.lhs) ||
					containsLabelInExpr(e->comma.rhs))
				return true;
			break;
		case ExprKind::Cast:
	...
	void Emitter::bindFunctionParams(Function& fn, const FuncDef* def,
																	 U32 paramBase) {
		for (U32 i = 0; i < def->params.size(); ++i) {
			...
			if (isAggregate(p.type)) {
				...
				declare(*p.name, Local{fn.declareLocal(*p.name, arg), nullptr, p.type,
															 false, false});
			}
		}
	}
