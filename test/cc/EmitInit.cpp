...
namespace rat::cc {
	const Expr* Emitter::peelAggregateCompound(const Expr* el) {
		if (el->kind == ExprKind::CompoundLit && !el->compound.isArray &&
				el->compound.init && el->compound.init->kind == ExprKind::InitList)
	...
	B32 Emitter::storeScalar(Function& fn, Node* slot, U32 off, CType dt,
													 const Expr* e) {
	...
	B32 Emitter::storeCharArray(Function& fn, Node* slot, U32 base, CType elem,
	...
	U32 Emitter::arrayInitOuterExtent(CType elem, const Expr* init) {
		...
		for (U32 i = 0; i < els.size();) {
			...
			if (des.isSet && des.next && isArrayType(elem) && des.next->isIndex) {
				U32 rowFree = (U32)des.next->index + 1 < rowLen
													? rowLen - ((U32)des.next->index + 1)
													: 0;
				U32 j = i + 1;
				for (; j < els.size() && !init->designators[j].isSet && rowFree > 0;
						 ++j, --rowFree)
					;
				i = j;
			} else if (isArrayType(elem) && el->kind != ExprKind::InitList &&
								 el->kind != ExprKind::CompoundLit &&
								 el->kind != ExprKind::StrLit) {
				U32 rowFree = rowLen;
				U32 j = i;
				for (; j < els.size() && rowFree > 0 &&
							 (j == i || !init->designators[j].isSet);
						 ++j, --rowFree)
					;
				i = j;
			} else {
	...
	B32 Emitter::initArrayRow(InitSink& sink, U32 off, CType elem,
														const Expr* init, const Designator& des, U32& i,
														U32& cur) {
		const List<Expr*>& els = init->args;
		const Expr* el = els[i];
	...
	B32 Emitter::initArrayInit(InitSink& sink, U32 base, CType elem, U32 count,
														 const Expr* init) {
		...
		for (U32 i = 0; i < els.size();) {
			...
			if (des.isSet && des.next) {
				if (isArrayType(elem)) {
					if (!initArrayInit(sink, off, arrayElem(elem), elem.array->count,
														 wrapNested(des.next, el)))
						return false;
			...
			if ((isStruct(elem) || isArrayType(elem)) &&
					el->kind != ExprKind::InitList && el->kind != ExprKind::CompoundLit &&
			...
			if (isArrayType(elem)) {
				if (elemIsChar && el->kind == ExprKind::StrLit) {
					if (!sink.charArray(off, arrayElem(elem), elem.array->count, el))
						return false;
				} else if (!initArrayInit(sink, off, arrayElem(elem), elem.array->count,
																	peelAggregateCompound(el)))
					return false;
			} else if (isStruct(elem)) {
	...
	B32 Emitter::initUnionInit(InitSink& sink, U32 base, const StructType* st,
														 const Expr* init) {
	...
	B32 Emitter::initStructInit(InitSink& sink, U32 base, const StructType* st,
															const Expr* init) {
		if (init->kind != ExprKind::InitList) {
		...
		for (U32 i = 0; i < els.size();) {
			...
			if (des.isSet) {
				...
				while (cur < fields.size() && fields[cur].anonMember &&
							 !fields[cur].anonFirst)
					++cur;
			}
			if (cur >= fields.size()) {
			...
			if (!des.isSet && f.anonMember && f.anonFirst) {
				...
				if (el->kind == ExprKind::InitList) {
					...
					return false;
				for (++cur; cur < fields.size() && fields[cur].anonMember &&
										!fields[cur].anonFirst;
						 ++cur)
					;
				continue;
			...
			if (des.isSet && des.next) {
				if (f.isArray) {
					if (!initArrayInit(sink, off, f.type, fcount,
														 wrapNested(des.next, el)))
						return false;
				} else if (isStruct(f.type)) {
					if (!initStructInit(sink, off, f.type.strukt,
															wrapNested(des.next, el)))
						return false;
				} else {
			...
			if (f.isArray && el->kind != ExprKind::InitList &&
					el->kind != ExprKind::StrLit) {
			...
			if (f.isArray) {
				if (el->kind == ExprKind::StrLit) {
					if (!sink.charArray(off, f.type, fcount, el))
						return false;
				} else if (!initArrayInit(sink, off, f.type, fcount,
																	peelAggregateCompound(el)))
					return false;
			} else if (isStruct(f.type)) {
				if (!initStructInit(sink, off, f.type.strukt,
	...
	B32 Emitter::initFlatObject(InitSink& sink, U32 base, CType ty,
	...
	B32 Emitter::initFlatStruct(InitSink& sink, U32 base, const StructType* st,
															const List<Expr*>& els, U32& pos,
															const List<Designator>* des) {
		B32 first = true;
		for (U32 fi = 0; fi < st->fields.size() && pos < els.size(); ++fi) {
			...
			if (f.isArray) {
				...
				if (e->kind == ExprKind::InitList) {
					...
					if (!sink.charArray(base + f.offset, f.type, f.count, e))
						return false;
				} else if (!initFlatArray(sink, base + f.offset, f.type, f.count, els,
																	pos, des))
					return false;
				if (st->isUnion)
					break;
			...
			if (f.isBitfield) {
				if (!sink.bitfield(base + f.offset, f.type, f.bitWidth, f.bitOffset,
													 els[pos]))
					return false;
				++pos;
				if (st->isUnion)
	...
	B32 Emitter::initFlatArray(InitSink& sink, U32 base, CType elem, U32 count,
														 const List<Expr*>& els, U32& pos,
														 const List<Designator>* des) {
		U32 esz = byteSize(elem);
	...
	B32 Emitter::StoreSink::bitfield(U32 off, CType dt, U32 width, U32 bitOff,
																	 const Expr* e) {
		if (e->kind == ExprKind::InitList) {
			if (e->args.empty())
				return true;
	...
	B32 Emitter::StoreSink::charArray(U32 base, CType elem, U32 count,
																		const Expr* e) {
	...
	B32 Emitter::StoreSink::structCopy(U32 off, CType ty, const Expr* e) {
		...
		if (!v.node)
			return false;
		Node* dst =
				off ? fn.add(slot, fn.constInt(emit.irType(emit.ctSize()), off)) : slot;
	...
	B32 Emitter::ImageSink::bitfield(U32 off, CType dt, U32 width, U32 bitOff,
																	 const Expr* e) {
		I64 v = 0;
		if (!emit.evalConst(e, v)) {
			emit.failNonConstInit();
	...
	B32 Emitter::ImageSink::charArray(U32 base, CType elem, U32 count,
																		const Expr* e) {
		if (elem.ptr != 0 || isStruct(elem) || elem.bits != 8) {
	...
	void Emitter::flatConsumeObject(CType ty, const List<Expr*>& els, U32& pos) {
		...
		if (isStruct(ty)) {
			...
			for (U32 fi = 0; fi < ty.strukt->fields.size() && pos < els.size();
					 ++fi) {
		...
		if (isArrayType(ty)) {
			...
			U32 cnt = ty.array->count;
			if (e->kind == ExprKind::InitList ||
					(isCharType(el) && e->kind == ExprKind::StrLit)) {
				++pos;
				return;
			}
	...
	U32 Emitter::flexElemCount(const StructType* st, const Expr* init) {
		if (st->isUnion || st->fields.empty() || !init ||
				init->kind != ExprKind::InitList)
			return 0;
		...
		for (U32 i = 0; i < els.size();) {
			...
			while (cur < st->fields.size() && st->fields[cur].anonMember &&
						 !st->fields[cur].anonFirst)
				++cur;
