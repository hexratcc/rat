...
namespace rat::cc {
	...
	B32 Emitter::evalConstUnary(ExprOp op, I64 v, CType opTy, I64& out,
															CType& ty) {
		switch (op) {
		case ExprOp::Pos:
			out = v;
	...
	B32 Emitter::evalConstBinary(ExprOp op, I64 a, CType aTy, I64 b, CType bTy,
															 I64& out, CType& ty) {
	...
	B32 Emitter::evalConstTyped(const Expr* e, I64& out, CType& ty) {
		...
		case ExprKind::Binary: {
			CType aTy, bTy;
			I64 a, b;
			if (!evalConstTyped(e->binary.lhs, a, aTy) ||
					!evalConstTyped(e->binary.rhs, b, bTy))
				return false;
		...
		case ExprKind::Ternary: {
			...
			if (!evalConstTyped(e->ternary.cond, c, cTy))
				return false;
			return evalConstTyped(c ? e->ternary.whenTrue : e->ternary.whenFalse, out,
														ty);
		}
		case ExprKind::Comma:
	...
	B32 Emitter::evalFloatConst(const Expr* e, long double& out) {
		...
		case ExprKind::Binary: {
			long double a, b;
			if (!evalFloatConst(e->binary.lhs, a) ||
					!evalFloatConst(e->binary.rhs, b))
				return false;
			switch (e->binary.op) {
			case ExprOp::Add:
				out = a + b;
		...
		case ExprKind::Ternary: {
			...
			return evalFloatConst(taken ? e->ternary.whenTrue : e->ternary.whenFalse,
														out);
		}
		default:
			return false;
