...
namespace rat::cc {
	namespace {
		...
		B32 isPunct(const PpToken& t, const char* s) {
			return t.kind == Pk::Punct && t.text == s;
		}
		...
		LexResult lexAll(const String& s, const List<U32>& lineOf,
										 const String& file) {
			LexResult r;
			size_t i = 0, n = s.size();
			B32 bolPending = true;
		...
		I64 parseCharConst(const String& txt) {
			...
			default:
				if (isOctalDigit(e)) {
					I64 v = e - '0';
					for (U32 k = 0; k < 2 && i < txt.size() && isOctalDigit(txt[i]);
							 ++k, ++i)
						v = v * 8 + (txt[i] - '0');
					return v;
				}
		...
		struct Eval {
			...
			Eval(const List<PpToken>& toks, String& e) : t(toks), err(e) {}
			...
			Val apply(const String& op, Val a, Val b) {
				...
				if (op == "*")
					return u ? Val{a.u * b.u, true}
									 : Val{(U64)((I64)a.u * (I64)b.u), false};
				if (op == "+")
					return u ? Val{a.u + b.u, true}
									 : Val{(U64)((I64)a.u + (I64)b.u), false};
				if (op == "-")
				...
				if (op == ">>")
					return a.isU ? Val{a.u >> (b.u & 63), true}
											 : Val{(U64)((I64)a.u >> (b.u & 63)), false};
				if (op == "<")
		...
		struct Preprocessor {
			...
			Preprocessor(const PpOptions& o) : opts(o) {}
			...
			B32 isBuiltinDynamic(const String& name) {
				return name == "__LINE__" || name == "__FILE__";
			}
			B32 isDefined(const String& name) {
				return macros.count(name) || isBuiltinDynamic(name);
			}
			...
			List<PpToken> substitute(const Macro& m, const List<List<PpToken>>& args,
															 const Set<String>& hs,
				...
				auto isVa = [&](const String& s) {
					return m.variadic && s == m.vaName;
				};
			...
			B32 gatherArgs(std::deque<PpToken>& work, List<List<PpToken>>& raw,
										 PpToken& rparen) {
				int depth = 1;
				List<PpToken> cur;
				for (;;) {
			...
			B32 mapArgs(const Macro& m, const List<List<PpToken>>& raw,
			...
			void requeueExpansion(List<PpToken>& r, const PpToken& invoker,
			...
			void doDefine(const List<PpToken>& toks) {
				...
				if (!m.body.empty() &&
						(isPunct(m.body.front(), "##") || isPunct(m.body.back(), "##"))) {
			...
			void doInclude(const List<PpToken>& restIn, const String& curDir,
										 B32 next = false) {
			...
			void doPragma(const List<PpToken>& rest, const String& path) {
				if (rest.size() == 1 && rest[0].kind == Pk::Id &&
						rest[0].text == "once") {
					pragmaOnce.insert(path);
				} else if (rest.size() >= 1 && rest[0].kind == Pk::Id &&
									 (rest[0].text == "push_macro" ||
										rest[0].text == "pop_macro")) {
					String mname;
			...
			List<PpToken> applyPragmaOperators(List<PpToken>& toks,
																				 const String& path) {
				List<PpToken> out;
				for (size_t i = 0; i < toks.size();) {
					if (toks[i].kind == Pk::Id && toks[i].text == "_Pragma" &&
							i + 3 < toks.size() && isPunct(toks[i + 1], "(") &&
							toks[i + 2].kind == Pk::Str && isPunct(toks[i + 3], ")")) {
						List<PpToken> body =
								lexFragment(destringize(toks[i + 2].text), path);
						doPragma(body, path);
						i += 4;
						continue;
					}
			...
			B32 handleConditional(const String& name, const List<PpToken>& rest,
														List<Cond>& stack) {
			...
			void installBuiltins() {
				...
				// GNU C extensions
				auto defineFrag = [&](const char* text) {
					doDefine(lexFragment(text, "<builtin>"));
				};
