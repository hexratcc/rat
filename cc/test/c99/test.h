...
/* ---- chibicc test/common, inlined ----
   The upstream suite compiles test/common as a separate translation
   unit and links it with each test. This single-file harness instead
   inlines it here via test.h. The system <stdio.h>/<stdlib.h> includes
   are dropped so the minimal prototypes above are used (they are
   intentionally incompatible with the real headers, exactly as in
   upstream where common and the tests never share a translation unit).
   <stdarg.h> is kept because add_all() needs va_list. ---- */
#include <stdarg.h>
